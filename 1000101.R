...
N <-1000
J <- 20
a.true <- runif(J,0.5,1.5)
b.true <- rnorm(J,0,1)
gamma.true <- rnorm(J,0,1)
alpha.true <- runif(J,0.5,1.5)
sigma.true <- 1/alpha.true
delta.true <- runif(1,0.5,1.5)
beta.true <- runif(1,0.5,1.5)
set.seed(101)
sim <- function(M){
  ...
  step.size.a <- step.size.b <-  step.size.alpha <- step.size.gamma <- rep(1/N,J)
  ...
  H1 <- function(x1,x2,j){
    ...
    L1 <-sum(((outer(XIJ[,j],log(pkj))+outer(1-XIJ[,j],log(1-pkj)))*post.theta*matrix(rep(D[,j] ,K),N,byrow=FALSE)) )
    return(L1)
  }
  ...
  for (m in 1:M){
    ...
    for (j in 1:J){
      ...
      round.a[j] <-(sum(outer(XIJ[,j],P.K1J[,j],"-")*(matrix(rep(theta.seq,N),N,byrow=TRUE)-par$b[j])*post.theta*matrix(rep(D[,j] ,K),N,byrow=FALSE)))
      ...
      round.b[j] <- (sum(-par$a[j]*outer(XIJ[,j],P.K1J[,j],"-")*post.theta*matrix(rep(D[,j] ,K),N,byrow=FALSE))+alphasquare[j]*par$beta*sum(STAND.POST*(D[,j]%x%matrix(1,K,K))*log.mean.tj)+par$alpha[j]*par$beta*sum(STAND.POST*(V.MATRIX[,j]%x%matrix(1,K,K))*nor))
      ...
      round.alpha[j] <-responseitem[j]/par$alpha[j]- (par$alpha[j]*sum(STAND.POST*(D[,j]%x%matrix(1,K,K))*log.mean.tj*log.mean.tj)+sum(STAND.POST*(V.MATRIX[,j]%x%matrix(1,K,K))*nor*leave.tj))
      ...
      round.gamma[j] <- (alphasquare[j]*sum(STAND.POST*(D[,j]%x%matrix(1,K,K))*log.mean.tj)+par$alpha[j]*sum(STAND.POST*(V.MATRIX[,j]%x%matrix(1,K,K))*nor))
      ...
      round.beta[j] <- (alphasquare[j]*sum(STAND.POST*(par$b[j]-matrix(rep(theta.seq,N*K),ncol=K,byrow = F))*(D[,j]%x%matrix(1,K,K))*log.mean.tj)+par$alpha[j]*sum(STAND.POST*(par$b[j]-matrix(rep(theta.seq,N*K),ncol=K,byrow = F))*(V.MATRIX[,j]%x%matrix(1,K,K))*nor))
      
      round.delta[j] <- -(alphasquare[j]*sum(STAND.POST*(matrix(rep(tau.seq,N*K),ncol=K,byrow = T))*(D[,j]%x%matrix(1,K,K))*log.mean.tj)+par$alpha[j]*sum(STAND.POST*(matrix(rep(tau.seq,N*K),ncol=K,byrow = T))*(V.MATRIX[,j]%x%matrix(1,K,K))*nor))
      
    }
    
    
    HH3.all <- sum(HH3)
    round.beta.all <- sum(round.beta)
    ...
    round.delta.all <- sum(round.delta)
  ...
  for (j in 1:J){
    ...
    i.b[j] <- (sum(par$a[j]*par$a[j]*matrix(rep(P.K1J[,j]*(1-P.K1J[,j]),N),N,byrow=TRUE)*post.theta*matrix(rep(D[,j] ,K),N,byrow=FALSE)))+responseitem[j]*par$alpha[j]*par$alpha[j]*par$beta*par$beta+par$alpha[j]*par$alpha[j]*par$beta*par$beta*sum(STAND.POST*(V.MATRIX[,j]%x%matrix(1,K,K))*MA)
    i.gamma[j] <- responseitem[j]*par$alpha[j]*par$alpha[j]+par$alpha[j]*par$alpha[j]*sum(STAND.POST*(V.MATRIX[,j]%x%matrix(1,K,K))*MA)
    i.alpha[j] <- responseitem[j]/(par$alpha[j]*par$alpha[j])+sum(STAND.POST*(D[,j]%x%matrix(1,K,K))*log.mean.tj1*log.mean.tj1)+sum(STAND.POST*(V.MATRIX[,j]%x%matrix(1,K,K))*MA*leave.tj1*leave.tj1)
  ...
  A.RESULT <-B.RESULT <- ALPHA.RESULT <- GAMMA.RESULT <-  matrix(NA,J,4)
  GAMMA.BETA.RESULT <- matrix(NA,J,4)
  A.RESULT[,1] <- a.true
  A.RESULT[,2] <-  par$a
  A.RESULT[,3] <- par$a-a.true
  
  A.RESULT[,4] <- SE.RE[,1] 
  B.RESULT[,1] <- b.true
  B.RESULT[,2] <-  par$b
  B.RESULT[,3] <- par$b-b.true
  B.RESULT[,4] <- SE.RE[,2] 
  ALPHA.RESULT[,1] <- alpha.true
  ALPHA.RESULT[,2] <-  par$alpha
  ALPHA.RESULT[,3] <- par$alpha-alpha.true
  
  ALPHA.RESULT[,4] <- SE.RE[,3] 
  GAMMA.RESULT[,1] <- gamma.true
  GAMMA.RESULT[,2] <-  par$gamma
  GAMMA.RESULT[,3] <-  par$gamma-gamma.true
  
  
  GAMMA.RESULT[,4] <- SE.RE[,4] 
  GAMMA.BETA.RESULT[1,] <- c(beta.true,par$beta,par$beta-beta.true,SE.RE[1,5])
  GAMMA.BETA.RESULT[2,] <- c(delta.true,par$delta,par$delta-delta.true,SE.RE[1,6])
  ESTIMATE <- rbind(A.RESULT,B.RESULT,ALPHA.RESULT,GAMMA.RESULT,GAMMA.BETA.RESULT)
  colnames(ESTIMATE) <- c("True","Est","Bias","SE")
...
P <- 2
REP <- replicate(P,sim(500))
write.csv(REP, file = "1000101out.csv")